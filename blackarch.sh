...
# 2 Encrypt Partition
cryptsetup luksFormat --perf-no_read_workqueue --perf-no_write_workqueue --type luks2 --cipher aes-xts-plain64 --key-size 512 --iter-time 5000 --pbkdf argon2id --hash sha3-512 /dev/nvme0n1p5
cryptsetup --allow-discards --perf-no_read_workqueue --perf-no_write_workqueue --persistent open /dev/nvme0n1p5 r00t

# 3 - Formatting the partitions:
# the first one is our ESP partition, so for now we just need to format it
#mkfs.vfat -F32 -n "EFI" /dev/nvme0n1p1
mkfs.btrfs -L ROOT /dev/mapper/r00t
...
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@ /dev/mapper/r00t /mnt
mkdir -p /mnt/{boot,home,var/cache/pacman/pkg,.snapshots,.swapvol,btrfs}
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@home /dev/mapper/r00t /mnt/home
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@pkg /dev/mapper/r00t /mnt/var/cache/pacman/pkg
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@abs /dev/mapper/r00t /mnt/var/abs
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@tmp /dev/mapper/r00t /mnt/var/tmp
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@srv /dev/mapper/r00t /mnt/srv
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvol=@snapshots /dev/mapper/r00t /mnt/.snapshots
mount -o compress=no,space_cache,ssd,discard=async,subvol=@swap /dev/mapper/r00t /mnt/.swapvol
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,autodefrag,subvolid=5 /dev/mapper/r00t /mnt/btrfs
...
pacstrap /mnt base base-devel linux linux-firmware intel-ucode btrfs-progs sbsigntools \
    neovim zstd go iwd networkmanager mesa vulkan-radeon libva-mesa-driver mesa-vdpau \
    docker libvirt qemu openssh refind zsh zsh-completions \
...
export sph1nxx=username
# Replace hostname with the name for your host
export blackarch=hostname
...
# - add user
useradd -mg users -G wheel,storage,power,docker,libvirt,kvm,audio,video, -s /bin/zsh $USER
passwd $USER
...
# else
sed -i 's/^HOOKS/HOOKS=(base systemd autodetect keyboard modconf block sd-encrypt resume filesystems fsck)/' mkinitcpio.conf
...
git clone https://github.com/Yannis4444/Matrix-rEFInd.git /boot/EFI/refind/themes/refind-matrix
...
sed -i 's/#scanfor internal,external,optical,manual/scanfor manual,external,optical/' /boot/EFI/refind/refind.conf
...
menuentry "Arch Linux" {
    ...
    options  "rd.luks.name=$(blkid /dev/nvme0n1p5 | cut -d " " -f2 | cut -d '=' -f2 | sed 's/\"//g')=crypt root=/dev/mapper/r00t rootflags=subvol=@ resume=/dev/mapper/r00t resume_offset=$( echo "$(btrfs_map_physical /.swapvol/swapfile | head -n2 | tail -n1 | awk '{print $6}') / $(getconf PAGESIZE) " | bc) rw quiet nmi_watchdog=0 add_efi_memmap initrd=/amd-ucode.img"
    submenuentry "Boot using fallback initramfs" {
        initrd /boot/initramfs-linux-fallback.img
    }
}
include themes/refind-matrix/theme.conf
EOF
